...
- **(TE-prefix property.)** A walk stored to `T_max` contains every shorter echo time as a leading prefix: the first `N_t' = ⌈TE'/Δt⌉` saves are a valid, converged walk to `TE' ≤ T_max`. A replayer MAY slice. Two consequences for a replayer and a producer. **An acquisition ends at its echo:** every replay operation of §6 -- the gradient phase, the relaxation and surface sums, the field gate -- is evaluated over the acquisition's own duration, and a walk stored beyond a shorter acquisition's echo contributes nothing to that acquisition (a 10 ms spin echo on a 20 ms pack carries `exp(-10 ms / T2)`, never `exp(-20 ms / T2)`). **A band-limited prefix is a re-encoding**, not a slice: a truncated representation (§9) stores `K` temporal modes over `T_max`, so its prefix to `TE'` is the decoded channels cut at `N_t'` and re-projected onto `K'` modes, starting from `K' = ⌈K · TE' / T_max⌉` -- the same **bands per second** (§9.4 rule 6) -- and certified as any lossy pack is (rule 4), against the parent's decoded prefix. The certificate decides, not the ratio: the truncation error of a pinned residual has a floor in the band count that does not shrink with the duration, so a producer raises `K'` (up to the parent's `K`) until the certificate passes and records every band tried (reference measurement: a 100 ms walk at `K = 48` prefixes to 25 ms at `K' = 12` but needs `K' = 12`, not 6, at 12.5 ms). The child MUST name its parent in `provenance.prefix` (the parent's `id` and content digest, `T_max`, `K`, and the cut `TE'`, `K'`), and a child whose certificate fails MUST be refused rather than written.
...
where `χ_k ∈ {0,1}` encodes transverse vs longitudinal periods of the sequence (a spin-echo is transverse throughout its encoding). The relaxation-weighted signal replaces `w_i → w_i · W_i` in §6.1. `T2[·]`, `T1[·]` are replay knobs the caller supplies, by pool of the embedded substrate specification (§8.5, §10); the pack carries none.
...
MUST use id `0` for the extra-cellular/free pool. Every other id MUST be defined: by the pool of that id in the embedded `substrate` specification (§10, SUBSTRATE.md), or, for a pack without one, in `provenance`. A producer MUST NOT store a tissue value -- `T2`, `T1`, surface relaxivity, susceptibility, bound-pool relaxation -- as a pack property: those are replay knobs (§2), and the place for their nominal values is the substrate specification. A replayer's default is the gradient alone; the **nominal replay** -- the specification's declared values, its `nominal_field_T` as `B0` -- is one explicit call away (SUBSTRATE.md §6), so that a published pack reproduces its paper by firing a pulse at it and never by a default the caller did not state. Per-pool replay knobs (`T2`, `T1`) are given by the pool names of the embedded specification and MUST cover every pool of it; a pack without a specification has no pool names, takes no per-pool value, and replays the gradient alone.
...
A conformant representation or codec MUST satisfy six rules:
...
```jsonc
{
  ...
  "compression": {                       // REQUIRED — codec descriptor (see CODEC_REGISTRY.md)
    ...
    "temporal_bandwidth_hz": 603.8        // RECOMMENDED — K / (2 T_max): the band as a frequency (§9.4 rule 6)
  },
  ...
  "provenance": {                        // RECOMMENDED — how the walk was made
    ...
    "prefix": null },                    // REQUIRED on a re-encoded prefix (§3): the parent's id and content
                                         //   digest, its T_max and K, and this child's TE' and K'; null otherwise
